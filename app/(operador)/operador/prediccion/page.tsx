...
export default function PrediccionPage() {
  ...
  const consultar = useCallback(async () => {
    ...
    try {
      ...
      const ests: EstDia[] = (json.estaciones ?? []).sort(
        (a: EstDia, b: EstDia) => b.demanda_dia - a.demanda_dia)
      setDatos(ests)
      setMeta(json.metadatos ?? null)
      setSelId(prev => ests.some(e => e.id === prev) ? prev : (ests[0]?.id ?? null))
    } finally {
      setLoading(false)
    }
  }, [dia])
  ...
  const enRiesgo    = datos.filter(e => e.faltan > 0)
  const demandaTotal = datos.reduce((s, e) => s + e.demanda_dia, 0)
  const bicisNecesarias = enRiesgo.reduce((s, e) => s + e.faltan, 0)
  const sel = datos.find(e => e.id === selId) ?? null
  const maxDemanda = Math.max(...datos.map(e => e.demanda_dia), 1)
  ...
  function mensaje(e: EstDia): { texto: string; tipo: 'critico' | 'ok' | 'futuro' } {
    if (esFuturo) {
      if (e.demanda_dia === 0) return { texto: 'Sin demanda prevista para este día.', tipo: 'ok' }
      return {
        texto: `Colocar ${e.faltan} ${e.faltan === 1 ? 'bicicleta' : 'bicicletas'} a primera hora — se prevén ${e.demanda_dia} viajes, con pico a las ${hh(e.hora_pico)}.`,
        tipo: 'futuro',
      }
    }
    if (e.faltan > 0) {
    ...
    if (e.demanda_dia === 0) return { texto: 'Sin demanda prevista para hoy.', tipo: 'ok' }
    return {
    ...
    <div className="min-h-screen bg-[#f8fafb] pb-10">
      ...
      <div className="px-8 py-5 space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex flex-wrap items-center gap-3">
          ...
          {meta && (
            <span className="ml-auto text-xs font-bold text-[#16a34a] bg-[#f0fdf4] border border-[#bbf7d0] px-3 py-2 rounded-xl">
              {fechaLabel}{esFuturo ? ' · día futuro' : ' · hoy'}
            </span>
          )}
        </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: 'Demanda prevista del día', value: loading ? '—' : demandaTotal, unit: 'viajes',
              Icon: TrendingUp, bg: '#f0fdf4', ic: '#16a34a' },
            { label: esFuturo ? 'Estaciones con demanda' : 'Estaciones en riesgo',
              value: loading ? '—' : (esFuturo ? datos.filter(e => e.demanda_dia > 0).length : enRiesgo.length),
          ...
          <div className="xl:col-span-3 bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100">
              <h2 className="font-bold text-[#0f2419] text-sm flex items-center gap-2">
                ...
                Estaciones por demanda {esFuturo ? 'prevista' : 'de hoy'}
              </h2>
                    ...
                    <BarChart data={sel.por_hora} margin={{ top: 10, right: 8, left: -22, bottom: 0 }}>
                      ...
                      <Bar dataKey="demanda" radius={[4, 4, 0, 0]} maxBarSize={22}>
                        {sel.por_hora.map((x, i) => (
                          <Cell key={i}
                            fill={x.hora === sel.hora_pico ? '#0f2419' : '#b2f746'}
        ...
        {meta && (
          <div className="px-4 py-3 rounded-xl flex items-start gap-3 bg-white border border-gray-100 shadow-sm">
            ...
            <p className="text-xs text-gray-500 leading-relaxed">
              ...
              El modelo aprende los patrones de demanda por estación, día y hora — mejora conforme se acumula más historial real.
            </p>
          </div>
        )}
      </div>
